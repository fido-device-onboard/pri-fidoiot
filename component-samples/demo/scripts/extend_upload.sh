...
#
#   ./extend_upload.sh -a SECP256R1 -m 127.0.0.1 -o 127.0.0.1 -s abcdef -u apiUser -k password1 -p password2
#   ./extend_upload.sh -a SECP384R1 -m 127.0.0.1 -o 127.0.0.1 -s abcdef -u apiUser -k password1 -p password2
#   ./extend_upload.sh -a RSA2048RESTR -m 127.0.0.1 -o 127.0.0.1 -s abcdef -u apiUser -k password1 -p password2
...
get_cert=$(curl --silent -w "%{http_code}\n" -D - --digest -u ${api_user}:${onr_api_passwd} --location --request GET "http://${onr_ip}:${onr_port}/api/v1/certificate?alias=${attestation_type}" -H 'Content-Type: text/plain' -o owner_cert_${attestation_type}.txt)
...
if [ "$get_cert_code" = "200" ]; then
    ...
    if [ "$get_voucher_code" = "200" ]; then
        ...
        upload_voucher=$(curl --silent -w "%{http_code}\n" -D - --digest -u ${api_user}:${onr_api_passwd} --location --request POST "http://${onr_ip}:${onr_port}/api/v1/owner/vouchers/" --header 'Content-Type: text/plain' --data-raw "$extended_voucher" -o ${serial_no}_guid.txt)
