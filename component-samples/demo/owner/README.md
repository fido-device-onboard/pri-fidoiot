...
| POST /api/v1/owner/svi/settings/ | Updates the various fields of `TO2_SETTINGS` table for ID=1 field.<br/> Example input looks like 'devicemtu:=2000,ownerthreshold:=8192',wgetModContentVerification:=<boolean> | | application/text| values based on the field(s) to be modified.| |
