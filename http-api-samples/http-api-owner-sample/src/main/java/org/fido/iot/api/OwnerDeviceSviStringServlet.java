...
public class OwnerDeviceSviStringServlet extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse resp)
    ...
    try {
      ...
      if (requestBody != null) {
        ...
        String[] deviceToOwnerSvi = requestBody.split(DELIMITER);
        if (deviceToOwnerSvi.length != 2) {
          getServletContext().log("Invalid request has been provided.");
          resp.setStatus(400);
          return;
        }

        ownerDbManager.addDeviceTypeOwnerSviString(ds, deviceToOwnerSvi[0], deviceToOwnerSvi[1]);
      }
    } catch (Exception exp) {
  ...
  protected void doDelete(HttpServletRequest req, HttpServletResponse resp)
    ...
    try {
      DataSource ds = (DataSource) getServletContext().getAttribute("datasource");
      new OwnerDbManager().removeDeviceSviString(ds, deviceType);
    } catch (Exception exp) {
      resp.setStatus(Const.HTTP_INTERNAL_SERVER_ERROR);
    }
  }
